...
def CalculatePressure():
    global sensor_initial_value, sensor_read_value
    return_pressure = (sensor_read_value - sensor_initial_value) * 91.00
    ...
    while True:
        try:
            ...
            if (motor_active):
                ...
                CSVData.append_pressure_reading(CalculatePressure())
    ...
    while True:
        ...
        if (motor_active):
            ...
            if (active_run_time > run_time):
                ...
                motor_active = False
                active_run_time = 0
                if (motor_active == False and not CSVData.is_empty()):
...
canvas.create_text(
    ...
    420.0,
    anchor="nw",
    text="Sensor Live Value Display",
    fill="#3B3D70",
    font=("Inter SemiBold", 10 * -1)
)
